...
from partialDerivator import cost_function, gradient_descent, get_normal_density_value
...
class UnivariateGaussianVariationalModel:
    ...
    def __gradient_descent_step(self, func, partial_var, learning_rate, var_value, data_point):
    ...
    def fit(self, x_bold_train, x_bold_val = None,learning_rate=0.01, epochs=100, patience=5):
        parameters = {
    ...
    def __has_improved(self, x_bold, partial_var, new_var_value, old_var_value):
        new_likelihood = sum(
            float(self.__get_log_normal_density(x, sigma=1, mu=partial_var).subs(partial_var, new_var_value))
            for x in x_bold
        )
        
        old_likelihood = sum(
            float(self.__get_log_normal_density(x, sigma=1, mu=partial_var).subs(partial_var, old_var_value))
            for x in x_bold
        )
    ...
    def __compute_loglikelihood(self, dist, parameters):
        #log_dist = sp.log(dist)
        log_dist = dist
        log_dist = lambdify(list(parameters.keys()), log_dist, "numpy")
        return np.sum(log_dist(**parameters))

    def __get_normal_density_value(self, x, sigma, mu):
        return sp.log((1 / (sqrt(2 * pi * sigma**2))) * exp(-0.5 * ((x - mu) ** 2)/(sigma**2)))
    
    def __get_log_normal_density(self, x, sigma, mu):
        return (
            -sp.Rational(1, 2) * sp.log(2 * sp.pi * sigma**2)
            - sp.Rational(1, 2) * ((x - mu)**2) / sigma**2
        )
